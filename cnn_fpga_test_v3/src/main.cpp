...
// Add after your existing #defines
...
int init_uart()
{
    ...
    XUartPs_SetBaudRate(&UartInstance, UART_BAUD_RATE);

    xil_printf("[UART] Ready at %d baud\r\n", UART_BAUD_RATE);
    return XST_SUCCESS;
}

u8 uart_recv_byte()
{
    while (XUartPs_IsReceiveData(UartInstance.Config.BaseAddress) == FALSE);
    return XUartPs_ReadReg(UartInstance.Config.BaseAddress, XUARTPS_FIFO_OFFSET);
}

void uart_send_byte(u8 byte)
{
    while (XUartPs_IsTransmitFull(UartInstance.Config.BaseAddress));
    XUartPs_WriteReg(UartInstance.Config.BaseAddress, XUARTPS_FIFO_OFFSET, byte);
}

void realtime_detection_mode()
{
    xil_printf("\r\n========================================\r\n");
    xil_printf("REAL-TIME DETECTION MODE\r\n");
    xil_printf("========================================\r\n");
    xil_printf("Waiting for images...\r\n\r\n");
    ...
    while (1) {
        ...
        u8 marker = uart_recv_byte();
        if (marker != START_MARKER) continue;

        // Receive 784 floats (3136 bytes)
        u8 *img_bytes = (u8*)input_image;
        for (int i = 0; i < 784 * 4; i++) {
            img_bytes[i] = uart_recv_byte();
        }

        // Wait for end marker
        u8 end = uart_recv_byte();
        if (end != END_MARKER) {
        ...
        // Run inference
        double inf_time = run_hybrid_inference(input_image, scores);
        int prediction = get_prediction(scores);

        // Send result
        uart_send_byte(START_MARKER);
        uart_send_byte((u8)prediction);
        uart_send_byte(END_MARKER);

        // Print status
        xil_printf("[%04d] Pred: %d | Time: %.1f ms\r\n",
                   frame_count++, prediction, inf_time);
    }
}
...
int main()
{
    xil_printf("\r\n\r\n");
    xil_printf("*****************************************\r\n");
    xil_printf("*   FPGA CNN REAL-TIME DETECTION        *\r\n");
    xil_printf("*****************************************\r\n\r\n");
    ...
    // Quick test
    xil_printf("[TEST] Running test inference...\r\n");
    float test_scores[10];
    double test_time = run_hybrid_inference(test_image, test_scores);
    int test_pred = get_prediction(test_scores);
    xil_printf("[TEST] Pred: %d | Time: %.1f ms\r\n\r\n", test_pred, test_time);

    // Enter real-time mode
    realtime_detection_mode();
