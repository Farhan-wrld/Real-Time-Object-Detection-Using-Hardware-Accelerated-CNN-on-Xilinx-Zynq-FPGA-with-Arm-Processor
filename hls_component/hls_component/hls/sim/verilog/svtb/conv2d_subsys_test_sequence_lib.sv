                    ...
                    fork
                        ...
                        begin
                            ...
                            for(int i=0; i<1; i++) begin
                                ...
                                logic[63:0] data64bit_weights[$];
                                logic[32-1:0] databusbit_weights[$];
                                ...
                                data64bit_weights.delete(); databusbit_weights.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_weights.push_back( ((refm.mem_blk_pages_gmem1.maxi_bundlevar_offset["weights"]+refm.mem_blk_pages_gmem1.page_ofst[refm.mem_blk_pages_gmem1.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                foreach(data64bit_weights[s]) databusbit_weights[s]=data64bit_weights[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_weights, 0, 28, 1);
                                ...
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_bias, 0, 40, 1);
                                data64bit_output_r.delete(); databusbit_output_r.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_output_r.push_back( ((refm.mem_blk_pages_gmem2.maxi_bundlevar_offset["output_r"]+refm.mem_blk_pages_gmem2.page_ofst[refm.mem_blk_pages_gmem2.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                foreach(data64bit_output_r[s]) databusbit_output_r[s]=data64bit_output_r[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_output_r, 0, 52, 1);
